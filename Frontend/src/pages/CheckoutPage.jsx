...
const CheckoutPage = () => {
    ...
    const [shippingInfo, setShippingInfo] = useState({
        fullName: "Usman Shams",
        email: "usman.shams@example.com",
        phone: "+91 98765 43210",
        address: "221B Baker St, Tech Corridor",
        city: "Bangalore",
        postalCode: "560001",
        paymentMethod: "Card"
    });
    ...
    const handleCopyReceipt = () => {
        const text = `NIKE DIGITAL INVOICE\nClient: ${shippingInfo.fullName}\nTotal: ₹${checkoutData.grandTotal}\nTXN-7451188122-BM`;
    ...
    return (
        <main className="CheckoutPageWrapper">
            <div className="CheckoutContainer">
                ...
                {/* LEFT: SHIPPING & BILLING ADDRESS FORM */}
                    ...
                    <div className="CheckoutCard">
                        ...
                        <div className="PaymentMethodsGrid">
                            {["Card", "UPI / QR", "Apple Pay", "NetBanking"].map((method) => (
                                ...
                                >
                                    <i className={`bx ${method === "Card" ? "bx-credit-card" : method === "UPI / QR" ? "bx-qr-scan" : "bx-wallet"}`}></i>
                                    <span>{method}</span>
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="CheckoutSummaryCard">
                        ...
                        <div className="OrderMiniItemsList">
                            {checkoutData.cartItems.map((item) => (
                                <div key={`${item.id}-${item.selectedSize}`} className="MiniItemRow">
                                    <img src={item.images?.[0] || item.image} alt={item.title} />
                                    <div className="MiniItemMeta">
                                        <strong>{item.title}</strong>
                                        <small>Qty: {item.quantity} | UK {item.selectedSize}</small>
                                    </div>
                        ...
                        <div className="LedgerBreakdown">
                            ...
                            {checkoutData.discount > 0 && (
                                <div className="BreakdownRow DiscountText">
                                    ...
                                    <span>-₹{checkoutData.discount.toLocaleString("en-IN")}</span>
                                </div>
                            )}
                            <div className="BreakdownRow">
                            ...
                            <div className="BreakdownRow">
                                <span>Delivery</span>
                                <span>{checkoutData.deliveryFee === 0 ? "FREE" : `₹${checkoutData.deliveryFee}`}</span>
                            </div>
                            ...
                            <div className="GrandTotalRow">
                                ...
                                <span className="FinalAmount">₹{checkoutData.grandTotal.toLocaleString("en-IN")}</span>
                            </div>
                        </div>

                        <button type="submit" form="checkout-form" className="PayNowButton">
                            <span>Authorize & Pay ₹{checkoutData.grandTotal.toLocaleString("en-IN")}</span>
                        ...
                        <div className={`POSThermalReceiptPaper ${receiptTorn ? "torn-effect" : ""}`}>
                            ...
                            <div className="ReceiptHeaderSection">
                                ...
                                <div className="ReceiptBrandLogo">
                                    <svg className="ReceiptSwoosh" viewBox="0 0 24 24" fill="currentColor">
                            ...
                            {/* Client & Red Stamped "PAID" Watermark */}
                            <div className="ReceiptClientAndStamp">
                                <div className="ClientMeta">
                                    <span className="ClientTag">CLIENT: {shippingInfo.fullName.toUpperCase()}</span>
                                    <div className="CardNumberMask">Visa-•••• 8842</div>
                                </div>

                                {/* 🔴 RED STAMPED "PAID" BADGE */}
                            ...
                            <div className="ReceiptItemsBlock">
                                {checkoutData.cartItems.map((item) => (
                                    <div key={`${item.id}-${item.selectedSize}`} className="ReceiptItemRow">
                            ...
                            <div className="ReceiptFinancialLedger">
                                ...
                                <div className="FinRow">
                                    <span>Gratuity (Tip)</span>
                                    <span>+₹{checkoutData.tip.toLocaleString("en-IN")}</span>
                                </div>
                                <div className="FinRow">
                                    <span>Tax (5%)</span>
                                    <span>₹{checkoutData.taxAmount.toLocaleString("en-IN")}</span>
                            ...
                            <div className="ReceiptGrandTotalRow">
                                <span>GRAND TOTAL</span>
                                <strong>₹{checkoutData.grandTotal.toLocaleString("en-IN")}</strong>
                            </div>
                            ...
                            {/* Sawtooth Serrated Perforated Tear Bottom */}
                            <div className="SawtoothTearEdge"></div>
                        </div>

                        {/* Bottom Success Message & Controller Buttons */}
